...
\label{sec:market_making}

Market making is a liquidity-provision problem in which an agent posts bid and ask
limit orders and is compensated through the bid--ask spread for supplying immediacy
\cite{garman_market_1976,ho_optimal_1981}. Let \(b_t\) and \(a_t\) denote the best
bid and best ask, and define the mid-price as \(m_t=(a_t+b_t)/2\). With cash \(C_t\)
and inventory \(Q_t\), the marked-to-market portfolio value is \(PV_t=C_t+Q_t m_t\).
The central risk variable is inventory: asymmetric fills can leave the agent long or
short, exposing it to future price movements and adverse selection. For this thesis,
the relevant theoretical point is that inventory is both a state variable and a risk
exposure. A risk-aware market maker should therefore adjust quotes to reduce large
positions rather than only maximise immediate spread capture.

\section{The Avellaneda--Stoikov Market-Making Model}
\label{sec:avellaneda_stoikov}

The Avellaneda--Stoikov model formalises inventory-aware quoting in a stylised
continuous-time limit-order-book setting \cite{avellaneda_high-frequency_2008}. In
the model, the mid-price \(S_t\) follows \(dS_t=\sigma dW_t\), and passive-order
arrivals decrease with quote distance, commonly written as
\(\lambda(\delta)=Ae^{-k\delta}\). Under exponential utility of terminal
marked-to-market wealth, the approximate reservation price is
\begin{equation}
    r_t = S_t - Q_t\gamma\sigma^2(T-t),
    \label{eq:as_reservation_price_short}
\end{equation}
where \(\gamma\) is the model risk-aversion parameter. A positive inventory lowers
the reservation price, making selling relatively more attractive; a negative inventory
has the opposite effect. The certainty-equivalent inventory term is proportional to
\(-\frac{\gamma}{2}Q_t^2\sigma^2(T-t)\), which gives the theoretical motivation for a
squared-inventory penalty. The reinforcement-learning coefficient
\(\rho_{\mathrm{MM}}\) used later is not numerically identical to \(\gamma\); it is a
discrete reward weight with the same economic role of discouraging excessive
inventory exposure.

\section{Order Execution in High-Frequency Trading}
\label{sec:order_execution}

Order execution concerns the liquidation or acquisition of a fixed parent order over a
finite horizon. Unlike the market maker, the execution agent is not primarily rewarded
for two-sided liquidity provision; it must reduce a remaining quantity while controlling
execution cost. At each decision step, it may use more passive actions, which can
reduce spread costs but risk non-completion, or more aggressive actions, which
improve completion probability but may increase slippage. The central risk variable is
therefore the remaining quantity \(X_t\). A large \(X_t\) indicates continuing exposure
to price uncertainty and a higher probability of a costly terminal rush.

\section{The Almgren--Chriss Optimal-Execution Model}
\label{sec:almgren_chriss}

The Almgren--Chriss model provides the classical risk--cost formulation for execution
\cite{almgren2000}. For a sell program, let \(x_0=X_0\), \(x_N=0\), and
\(n_k=x_{k-1}-x_k\) denote the amount traded in interval \(k\). With linear impact,
expected implementation shortfall \(E(x)\) increases with trading speed, while
execution risk is
\begin{equation}
    V(x)=\sigma^2\tau\sum_{k=1}^{N}x_k^2.
    \label{eq:ac_variance_short}
\end{equation}
The optimal schedule minimises \(E(x)+\lambda V(x)\), where \(\lambda\geq 0\) is the
classical execution risk-aversion parameter. Larger \(\lambda\) or volatility \(\sigma\)
produces a more front-loaded schedule; larger temporary impact \(\eta\) favours slower
execution, with urgency approximately governed by
\(\kappa\approx\sqrt{\lambda\sigma^2/\eta}\). This justifies penalising squared
remaining quantity in the learned execution reward. As with market making,
\(\rho_{\mathrm{EX}}\) is a discrete RL reward coefficient, not the continuous-time
parameter \(\lambda\).

\section{Reinforcement Learning and Independent PPO}
\label{sec:ippo}

Reinforcement learning represents both trading tasks as sequential decision problems.
At time \(t\), an agent observes \(o_t\), chooses an action \(a_t\), receives reward
\(r_t\), and updates a policy \(\pi_\theta(a_t\mid o_t)\) to maximise expected return.
In the multi-agent case, the next order-book state depends on the joint actions of all
agents, so each agent learns in an environment partly shaped by the other.
Independent PPO (IPPO) applies PPO separately to each agent, with separate policy
and value networks \cite{de_witt_independent_2020}. This is suitable here because
the market-making and execution agents have different observations, action spaces,
and rewards, while their realised outcomes remain coupled through the shared limit
order book.

\section{JaxMARL-HFT as the Experimental Environment}
\label{sec:jaxmarl_hft}

JaxMARL-HFT provides the replay-based multi-agent limit-order-book environment
used in this thesis. It extends JaxMARL with a JAX-LOB-based market simulator,
supports heterogeneous agents, and combines agent-generated orders with historical
market-by-order messages before matching
\cite{mohl_jaxmarl-hft_2025,frey_jax-lob_2023,rutherford_jaxmarl_2023}. The
framework is used as a scalable experimental testbed rather than as a new simulator
or a complete endogenous market model: the historical background order flow does
not adapt to the learned agents. The two risk-adjusted rewards studied in the
experiment are
\begin{equation}
    r_t^{\mathrm{MM}} = r_{t,\mathrm{base}}^{\mathrm{MM}} - \rho_{\mathrm{MM}}Q_t^2,
    \qquad
    r_t^{\mathrm{EX}} = r_{t,\mathrm{base}}^{\mathrm{EX}} - \rho_{\mathrm{EX}}X_t^2
    - \zeta_{\mathrm{EX}}\mathbf{1}_{\{t=T\}}|X_T|.
    \label{eq:unified_rl_rewards}
\end{equation}
Thus, \(Q_t\) and \(X_t\) are the risk variables inherited from classical market-making
and execution theory, while \(\rho_{\mathrm{MM}}\) and \(\rho_{\mathrm{EX}}\) are the
controlled experimental coefficients.
